 ...
 *
 * For each point given by the parameters, the closest pixel will be plotted
 * (ie. floating point coordinates are rounded to the closest integer).
 *
 * @param points the array of points
 * @param nb the number of pixels to plot.
 * Caution: the number of elements in the array must be large enough so that accessing this number of
 * pixels will not exceed the array size.
 *
 */
extern void PlotPixels(point points[], int nb);
 ...
 * Caution: the number of elements in the array must be large enough so that accessing this number of
 * points will not exceed the array size.
 *
 */
extern void DrawPoints(point points[], int nb);
 ...
 * @see enum line_algo
 */
extern void DrawLine(point p0, point p1x, enum line_algo algo);
 ...
 * Caution: this number must not be larger than the size of the array.
 ...
 */
extern void GenerateSquare(point points[], int nb, unsigned int seed);
 ...
 * You will reproduce a deterministic generation by using the same seed.
 */
extern void GenerateGalaxy(point points[], int nb, unsigned int seed);
 ...
 */
extern void DrawConvexHull(point points[], int nb_total_points, int nb_hull_points);
