...
void DrawLine(point p0, point p1, enum line_algo algo)
{
    switch (algo) {
        case BRESENHAM: PlotBresenhamLine((int)(p0.x+0.5), (int)(p0.y+0.5), (int)(p1.x+0.5), (int)(p1.y+0.5)); break;
        case GUPTA_SPROULL: DrawGuptaSproullLine((int)(p0.x+0.5), (int)(p0.y+0.5), (int)(p1.x+0.5), (int)(p1.y+0.5)); break;
        case WU: DrawWuLine(p0.x, p0.y, p1.x, p1.y); break;
    }
    UpdateScreen();
}
...
void PlotPixels(point points[], int nb)
{
    int i;
    for (i=0; i<nb; i++)
        IntensifyPixel((int)(points[i].x+0.5), (int)(points[i].y+0.5), MAX_INTENSITY);
    UpdateScreen();
}

void DrawPoints(point points[], int nb)
{
    int i;
    for (i=0; i<nb; i++)
...
void GenerateSquare(point points[], int nb, unsigned int seed)
{
    srand(seed);
...
void GenerateGalaxy(point points[], int nb, unsigned int seed)
{
    int i;
    srand(seed);
    ...
    for (i=0; i<nb; i++) {
        ...
        double angle = rand();
        points[i].x = window_width/2 + radius*cos(angle);
        points[i].y = window_height/2 + radius*sin(angle);
    }
}

void DrawConvexHull(point points[], int nb_points, int nb_hull_points)
{
    EmptySpace();
    int i;
    ...
    for (int i=0; i<nb_hull_points-1; i++)
         DrawWuLine(points[i].x, points[i].y, points[i+1].x, points[i+1].y);
    // draw segment between last and first points to close the polygon
    DrawWuLine(points[nb_hull_points-1].x, points[nb_hull_points-1].y,
