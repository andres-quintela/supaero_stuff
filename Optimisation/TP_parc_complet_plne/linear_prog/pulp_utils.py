...
class Params:
    def __init__(self):
            ...
            self.solveur="cplex"#coin
            self.logSolveur="on"
            self.solveurToleranceRelative=0.01
            self.solveurMaxTime=300.0
            self.pathCplex="/usr/local/ilog/cplex-1263/cplex/bin/x86-64_linux/cplex"
...
def getResultsModel(pb,model,name):
    ...
    #sum of hydroplants
    results.prod_vars_solution["cumulHydro"]=[]
    for t in pb.time_steps:
        results.prod_vars_solution["cumulHydro"].append(sum ([model.prod_vars[t][usine].varValue for usine in pb.hydro_plants]))
        
    results.prod_vars_solution["cumulThm"]=[]
    for t in pb.time_steps:
        results.prod_vars_solution["cumulThm"].append(sum ([model.prod_vars[t][usine].varValue for usine in pb.thermal_plants]))
...
def updateResultsWithSubPbm(sous_pb,pb,model,results):
    ...
    results.prod_vars_solution["cumulHydro"]=[]
    for t in pb.time_steps:
        results.prod_vars_solution["cumulHydro"].append(sum ([results.prod_vars_solution[usine.name][t] for usine in pb.hydro_plants]))
        
    results.prod_vars_solution["cumulThm"]=[]
    for t in pb.time_steps:
        results.prod_vars_solution["cumulThm"].append(sum ([results.prod_vars_solution[usine.name][t] for usine in pb.thermal_plants]))
...
def printResults(pb, model, name,electricity_prices,results):
        
    programs={"cumulThm":results.prod_vars_solution["cumulThm"],"cumulHydro":results.prod_vars_solution["cumulHydro"]}
    charts.display_EOD(programs, pb, model.indicators, name, pb.demand,electricity_prices)
...
def solve(model, name,params=Params()):
    ...
    if (params.solveur=="coin"):
        if (params.logSolveur=="on"):
            model.lp.setSolver(solvers.COIN_CMD(path,msg=1,fracGap=params.solveurToleranceRelative,maxSeconds=params.solveurMaxTime,keepFiles=1))
        else:
            model.lp.setSolver(solvers.COIN_CMD(path,msg=0,fracGap=params.solveurToleranceRelative,maxSeconds=params.solveurMaxTime,keepFiles=1))
    else:
...
def getAllVariables(model,name):
     ...
     charts.displayResultTable(sortedKeys,resultsVariable,"variables_"+name)
...
def getAllConstraints(model,name):
     ...
     charts.displayResultTable(sortedKeys,resultsConstraints,"constraints_"+name)
