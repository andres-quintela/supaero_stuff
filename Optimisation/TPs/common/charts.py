...
def plot_stock_management(pb, program, bellman_values, plot_title="Dynamic Programming"):
    defaults.height=400
    defaults.width = 800
    programs = {"stock": program}
    progs = plot_lines(programs, lambda i : str(i))
    prices = plot_lines ( { "base" : pb.base_prices[:pb.number_of_days], "peak" : pb.peak_prices[:pb.number_of_days]},
                         lambda i : str(i),
                         ylabel="euros/MWh")
...
#    indicators = ["initial_stock", "startup_cost", "quadratic_cost"
#                  , "maximum_increase_rate", "maximum_decrease_rate"]
#
#    data = dict(
#        indicator=[indicator for indicator in indicators],
#        value=[thermal_plant.__dict__[indicator] for indicator in indicators ]
#    )
#
#    print(data)
...
#    area3 = DataTable(source=source, columns=columns, width=400, height=280)
    ...
    return column(progs, prices, bellman_table)
...
def display_EOD(programs, pb, indicators, name):
    ...
    output_file("results.html", title=name, mode="inline")

    save(row(area1, area2,area3))
